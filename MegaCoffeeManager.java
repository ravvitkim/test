...
public class MegaCoffeeManager {
    ...
    private static void admin() {
        ...
        while (true) {
            ...
            System.out.println("1.메뉴 가격 수정하기 2. 메뉴추가하기 3.정산하기 4.돌아가기");
            int choice = sc.nextInt();
            switch (choice) {
                case 1:
                    //메뉴가격만 수정
                    menuUpdate();  //위치찾기 ctrl + 왼쪽마우스클릭
                    break;
                case 2:
                    //세새로운 메뉴추가 (단,총5개까지만)
                    menuInsert();
    ...
    private static void menuInsert() {
        ...
        int ableCount = 0;
        //제품배열을 순회하면서 비어있지 않은 제품 수를 구한다.
        for (int i = 0; i < MENU_COUNT; i++) {
            if (products[i] != null) {
                ableCount++;
            }
        }
        //전체 배열의 크기에서 뺀다 ->추가할수있는제품수
        System.out.println("현재 추가할 수 있는 제품의수 : " + (MENU_COUNT - ableCount));
        //추가할 곳이 없으면 관리자메뉴로 돌려보낸다.
...
//                        price[MENU_COUNT-ableCount] = newPrice;
    ...
    private static void viewMenu() {
        ...
        for (int i = 0; i<MENU_COUNT; i++){
            if (products[i] != null){
                String strMenu = products[i] + " / " + price[i];
                System.out.println(strMenu);
            }
        }
        System.out.println("---------------------------");
